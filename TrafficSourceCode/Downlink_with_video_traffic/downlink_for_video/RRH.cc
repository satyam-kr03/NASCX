...
private:
    ...
    vector<vector<double>> storesize_bits;        // remaining size of each packet in BITS
    ...
    vector<vector<double>> storemse;              // MSE per packet (from TrafficSource)
    ...
    cStdDev delayrec, PFrec,error;          // mean delay + PF stats
    vector<int> flag;
    vector<double> avgThroughput;     // bits/slot EWMA

    // CSV logging
    ofstream csv;
    const double DELAY_BOUND_S = 0.005; // 5 ms

    // Success metrics
    long packets_tx_total,total_pac = 0;
    long packets_tx_under_bound, total_pac_s = 0;
    double success_rate_pct = 0;
    int success = 0;

    // <-- NEW: Error accumulation (per UE)
    vector<double> mseSumPerUE;   // sum of mse_contrib (only when delay > bound)
    vector<long>   pktCntPerUE;   // number of transmitted packets per UE
    double totalMeanErr,avgMeanErr,mse,mse_contrib,meanErrU= 0.0;
    int counted = 0;
...
void RRH::initialize() {
    ...
    storemse.resize(numxr);

    flag.assign(numxr, 0);
    avgThroughput.assign(numxr, 1e-9);

    mseSumPerUE.assign(numxr, 0.0);
    pktCntPerUE.assign(numxr, 0);
    ...
    // Open CSV and write header (extended with mse fields)
    csv.open("rrh_packet_log.csv", ios::out);
    csv << "user,packet_id,size_bytes,delay_ms,success,mse,mse_contrib\n";
    csv.flush();
}
...
void RRH::handleMessage(cMessage *msg) {
    if (strcmp(msg->getName(), "data") == 0) {
        ...
        long   id         = pkt->getId();
        int    size_bytes = pkt->getByteLength();
        double size_bits  = pkt->getBitLength();
        double mse        = pkt->hasPar("mse") ? pkt->par("mse").doubleValue() : 0.0;
    ...
    for (int i = 0; i < numgroup; i++) {
        ...
        for (int j = 0; j < numxr; j++) {
            if (!storesize_bits[j].empty()) {
                ...
                // PF with delay in denominator (your existing form)
                double metric = (bits_per_sec / avgThroughput[j]);
                PFrec.collect(metric);
        ...
        if (maxind != -1) {
            ...
            while (!storesize_bits[maxind].empty() && transmitted_bits < maxrate_bits_per_slot) {
                ...
                if (remaining_head > capacity_left) {
                    ...
                    // Compute delay for this completed packet (keep your original logic)
                    double arrival_s = storearrival_s[maxind].front();
                    double delay_s = simTime().dbl() - arrival_s;

                    total_pac ++;
                    if (delay_s <= DELAY_BOUND_S){
                        error.collect(0);
                        total_pac_s ++;}
                    else
                    {
                        error.collect(storemse[maxind].front()); }

                    long pid = storepktid[maxind].front();
                    int size_bytes = storeorig_bytes[maxind].front();

                    // Update success + CSV + MSE accounting (happens BEFORE pop)
                    logPacketRow(maxind, pid, size_bytes, delay_s);

                    // Pop this packet from all queues (including MSE)
                    ...
                    storemse[maxind].erase(storemse[maxind].begin()); // <-- NEW
                }
            }
...
void RRH::logPacketRow(int user, long pktid, int size_bytes, double delay_s) {
    // Keep your delay logic; compute success from it
    success = (delay_s <= DELAY_BOUND_S) ? 1 : 0;
    double delay_ms = delay_s * 1000.0;

    // ---- Success metrics ----
    packets_tx_total += 1;
    if (success) packets_tx_under_bound += 1;
    success_rate_pct = (100.0 * (double)packets_tx_under_bound / (double)packets_tx_total);

    // ---- MSE accounting (per packet, per UE) ----
    mse = storemse[user].front();                 // current packet's MSE
    mse_contrib = success ? 0.0 : mse;            // add only if delay > bound
    mseSumPerUE[user] += mse_contrib;                    // accumulate for this UE
    pktCntPerUE[user] += 1;                              // count transmitted packet


    for (int u = 0; u < numxr; ++u) {
        if (pktCntPerUE[u] > 0) {
            meanErrU = mseSumPerUE[u] / (double)pktCntPerUE[u];
            totalMeanErr += meanErrU;
            counted++;
           }
       }
    avgMeanErr = (totalMeanErr / counted);
    // Record delay as scalar stat too (kept)
    delayrec.collect(delay_s);

    // Write CSV line (extended)
    csv << user << ","
        << pktid << ","
        << size_bytes << ","
        << fixed << setprecision(6) << delay_ms << ","
        << success << ","
        << setprecision(10) << mse << ","
        << mse_contrib << "\n";
    csv.flush();
...
void RRH::finish() {
    ...
    // PF stats
    EV << "min PF = " << PFrec.getMin() << endl;
    EV << "max PF = " << PFrec.getMax() << endl;
    EV << "mean PF = " << PFrec.getMean() << endl;

    EV << "mean_error = " << error.getMean() << endl;
    EV << "success = " <<(double)total_pac_s/total_pac<< endl;

    // Success stats
    recordScalar("packets transmitted (total)", (double)packets_tx_total);
    recordScalar("packets under delay bound", (double)packets_tx_under_bound);
    recordScalar("success rate (%)", (double)success_rate_pct);

    //EV << "packets transmitted (total) = " << packets_tx_total << endl;
    //EV << "packets under delay bound   = " << packets_tx_under_bound << endl;
    //EV << "success rate (%)            = " << success_rate_pct << endl;

    recordScalar("avg_mean_error", avgMeanErr);
    //EV << "avg_mean_error = " << avgMeanErr << endl;
