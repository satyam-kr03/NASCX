...
df = pd.read_csv('traffic_files/pca_sweep_summary.csv')

# 2. Define the target parameters
fps = 60
target_mbps = 30

# Calculate the target bytes for 80 components
# 30 Mbps = 30,000,000 bits/sec
# Target bytes per frame = (30,000,000 bits/sec / 60 frames/sec) / 8 bits/byte
target_bytes_80_comp = (target_mbps * 1e6) / (fps * 8) 
...
size_80_old = df[df['components'] == 80]['size_bytes'].mean()
...
# 5. Linear Rescaling
# Determine the scaling factor to match the 80-component target
scaling_factor = target_bytes_80_comp / size_80_old
...
output_filename = 'traffic_files/pca_sweep_summary.csv'
