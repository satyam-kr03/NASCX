...
def prepare_training_targets(df: pd.DataFrame, num_users: int):
    ...
    optimal_idx = df_n.groupby(state_cols)["total_error"].idxmin()
...
# ---------------------------------------------------------------------------
# Dataset with optional Gaussian augmentation
# ---------------------------------------------------------------------------
class CompressionDataset(Dataset):
    ...
    augment  : if True, add N(0, noise_std) to inputs each epoch
    copies   : how many noisy copies to add per original sample
    noise_std: std of augmentation noise (in normalised units)
    """
    ...
    def __init__(
        ...
        if augment and copies > 0:
            ...
            noisy  = [X_np + rng.normal(0, noise_std, X_np.shape)
                      for _ in range(copies)]
            X_np   = np.vstack([X_np] + noisy)
            Y_np   = np.tile(Y_np, (copies + 1, 1))
...
class MultiUserCompressionNet(nn.Module):
    ...
    Input  : (B, 2*num_users)  interleaved [cqi0, fps0, cqi1, fps1, ...]
    ...
    def __init__(self, num_users: int, num_classes: int = NUM_CLASSES):
        ...
        self.heads = nn.ModuleList([
            nn.Linear(32, num_classes) for _ in range(num_users)
        ])
...
def train_model(
    ...
    ds     = CompressionDataset(X_train, Y_train,
                                augment=True, copies=aug_copies,
                                noise_std=noise_std)
    ...
    print(f"  Training {num_users}-user model  "
          f"({len(ds):,} samples after augmentation, {epochs} epochs)...")
...
def evaluate_model(
    ...
    ds     = CompressionDataset(X_test, Y_test, augment=False)
...
def predict_components(
    ...
    raw_state: list,     # un-scaled [cqi0, fps0, cqi1, fps1, ...]
    device:    str = "cpu",
) -> list:
...
def train_all(
    ...
    for n in num_users_list:
        ...
        model = train_model(
            ...
            aug_copies=aug_copies, noise_std=noise_std,
            device=device,
        )
...
if __name__ == "__main__":
    CSV_PATH = "datasets_pca/random_cl_dataset_clean.csv"
    SAVE_DIR = "./models"
    ...
    for N_USERS in range(2, 11):
        train_all(
            ...
            batch_size     = 64,
            lr             = 1e-3,
            aug_copies     = 9,      # each state → 10 training examples
            noise_std      = 0.15,   # noise in normalised units (~0.15σ)
            save_dir       = SAVE_DIR,
            device         = DEVICE,
        )
        ...
        # raw_state = [14, 60, 10, 60, 15, 60]   # [cqi0, fps0, cqi1, fps1, cqi2, fps2]
