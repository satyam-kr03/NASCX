...
Simplified pipeline (inline model queries during simulation):
  ...
  2. Run static simulations for every compression level (5,10,...,80).
...
# Default path for comparison (evaluation) traffic files
COMPARISON_TRAFFIC_SUBPATH = "comparison/traffic_files/pca"
BASELINE_TRAFFIC_SUBPATH = "compression/traffic_files/pca"

COMP_LEVELS = list(range(5, 81, 5))
MAX_FRAMES = 500

FPS_OPTIONS = [60]              # Per-user frame rate choices - Moderate load

# file prefix for sweep summaries
...
def read_user_summary(run_dir: Path, user_idx: int) -> dict | None:
    """Read per-user summary CSV generated by XRTrafficReceiver (if available)."""
    ...
    try:
        ...
        total_frames = _to_int(row.get("total_frames", 0), 0)
        received_frames = _to_int(row.get("received_frames", 0), 0)
        late_frames = _to_int(row.get("late_frames", 0), 0)
        lost_frames = _to_int(row.get("lost_frames", 0), 0)
...
def count_missing_only_lost_rows(df: pd.DataFrame) -> int:
    """
    Count synthetic rows appended by detectLostFrames():
      i,0,0,0,0,0,-1,0,Elost,deadline,0,0,0,0.0,0
    """
    if df is None or df.empty:
        return 0

    required_cols = ["components", "mse", "genTime", "recvTime", "delay_ms"]
...
def _extract_user_metrics(
    ...
    summary = read_user_summary(run_dir, user_idx)
    missing_only_lost = count_missing_only_lost_rows(df)

    if summary is not None:
        mean_err = summary["mean_effective_error"]
        on_time = summary["on_time_ratio"]
        mean_delay = summary["mean_delay_ms"]
        ...
        incomplete_lost = max(0, int(lost_frames) - int(missing_only_lost))
        eval_source = "summary"
    else:
        ...
        expected_frames = int(df["frameNumber"].max()) if ("frameNumber" in df.columns and not df.empty) else 0
        received_frames = int((df["delay_ms"] >= 0).sum()) if "delay_ms" in df.columns else len(df)
        if "delay_ms" in df.columns and "receivedOnTime" in df.columns:
            late_frames = int(((df["delay_ms"] >= 0) & (df["receivedOnTime"] == 0)).sum())
        else:
            late_frames = 0
        lost_frames = int((df["delay_ms"] < 0).sum()) if "delay_ms" in df.columns else 0
        delivery_ratio = float(received_frames / expected_frames) if expected_frames > 0 else float("nan")
        loss_ratio = float(lost_frames / expected_frames) if expected_frames > 0 else float("nan")
        incomplete_lost = 0
    ...
    return {
        ...
        "incomplete_lost": incomplete_lost,
...
def assemble_comparison(
    ...
    if model_data:
        ...
        for i in range(num_users):
            if i in model_data:
                df = model_data[i]
                rows.append(_extract_user_metrics(
                    model_run_dir,
                    i,
                    df,
                    video_assignments[i],
...
def main():
    ...
    parser.add_argument("--sim-time", type=int, default=50,
                        help="Simulation time in seconds (default: 50)")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed (default: 42)")
    parser.add_argument("--server-url", default=MODEL_SERVER_URL,
                        help=f"Model server URL (default: {MODEL_SERVER_URL})")
    parser.add_argument("--traffic-dir", default=None,
                        help="Path to comparison traffic files (default: comparison/traffic_files/pca)")
    ...
    if args.traffic_dir:
        ...
        TRAFFIC_DIR = SCRIPT_DIR.parent / COMPARISON_TRAFFIC_SUBPATH

    # If the comparison folder does not exist, fall back to the baseline traffic files but warn.
    if not TRAFFIC_DIR.exists():
        fallback_dir = SCRIPT_DIR.parent / BASELINE_TRAFFIC_SUBPATH
        if fallback_dir.exists():
            print(f"  [WARN] Comparison traffic dir '{TRAFFIC_DIR}' not found. Falling back to baseline traffic dir '{fallback_dir}'.")
            TRAFFIC_DIR = fallback_dir
        else:
            print(f"  [ERROR] No traffic files found in '{TRAFFIC_DIR}' or fallback '{fallback_dir}'.")
            sys.exit(1)
    ...
    try:
        ...
        print(f"  Server OK: device={health['device']}, "
              f"max_users_supported={health.get('max_users_supported', 0)}")
        if num_users > health.get("max_users_supported", 0):
            print(f"  [WARN] Server supports up to {health.get('max_users_supported')} users, but {num_users} requested!")
            return
    except Exception as e:
