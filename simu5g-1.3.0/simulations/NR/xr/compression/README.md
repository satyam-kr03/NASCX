...
This document provides a detailed overview of the Principal Component Analysis (PCA) video compression pipeline located in the `compression/` directory.
...
The entry point for this process is `main.py`.
...
For each frame, the system performs validation matching component counts ($K \in {5, 10, ..., 80}$):
