...
from constants import DEFAULT_COMPONENTS, DEFAULT_IMG_SIZE
...
def evaluate_compression(
    ...
    for frame_idx, frame_uint8 in stream_test_frames(video_path, test_indices, img_size):
        ...
        # Compute MSE for all component counts
        mses = {}
        for n_comp in components_list:
        ...
        # Compute error_at_k80 and error_ratio
        error_at_k80 = mses.get(80, 0.0)
        mse_at_k5 = mses.get(5, 0.0)
        error_ratio = mse_at_k5 / error_at_k80 if error_at_k80 > 0 else 0.0
        ...
        if max_test_frames is not None and done >= max_test_frames:
            logging.info(f"  Reached limit of {max_test_frames} test frames. Stopping evaluation.")
            break
