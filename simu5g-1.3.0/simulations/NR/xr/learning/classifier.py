...
def verify_class_mapping() -> None:
    """Ensure class/component conversions are consistent for valid levels."""
    for cl in CL_LEVELS:
        cls = components_to_class(cl)
        if class_to_components(cls) != cl:
            raise ValueError(f"Invalid class mapping for components={cl}")
...
def compute_oracle_targets(df: pd.DataFrame, num_users: int) -> pd.DataFrame:
    """Compute per-frame oracle targets based on normalized error and fairness."""
    ...
    # Normalize error by frame_rate so high-FPS users aren't underweighted
    normalized_err_cols = []
    ...
    df_n["total_cost"] = df_n["total_error_scaled"] + (0.15 * df_n["variance_penalty_scaled"])
...
def build_padded_features(
    ...
    """Build padded feature/label/mask matrices for a given num_users."""
    comp_cols = [f"user{i}_components" for i in range(num_users)]
...
def prepare_training_targets(df: pd.DataFrame, num_users: int, max_users: int = MAX_USERS):
    """
    For each unique network state, find the component configuration that
    minimised total effective error across all users (the grid-search oracle).

    Returns
    -------
    X : DataFrame  (n_states, 21*max_users + 2)  — un-scaled state features (padded)
    ...
    X, Y, M = build_padded_features(opt, num_users, max_users)

    avg_target_components = opt[[f"user{i}_components" for i in range(num_users)]].mean().mean()

    print(f"  [{num_users} users] {len(X)} unique states  "
          f"(from {len(df[df['num_users'] == num_users]):,} total rows)")
...
class CompressionDataset(Dataset):
    ...
    Parameters
    ----------
    X        : scaled input features  (n, 21*max_users + 2)
...
def predict_with_probabilities(
    ...
    """Return components and class probabilities for each active user."""
    model.eval()
    ...
    for i in range(num_users):
        ...
        components.append(class_to_components(pred_idx))
