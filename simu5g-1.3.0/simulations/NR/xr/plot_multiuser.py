        ...
        for i, bar in enumerate(bars):
            ...
            if np.isfinite(h):
                ax.text(
                    ...
                    h * 1.01 if h >= 0 else h * 0.99,
                    f"{h:{fmt}}",
                    ha="center", va="bottom", fontsize=7, rotation=0,
                )

    # Annotate actual CL numbers on the min/mid/max bars
    for i, n in enumerate(user_counts):
        row = agg_df.loc[n]
        for j, cl_col in enumerate(["min_cl", "mid_cl", "max_cl"]):
            cl = int(row[cl_col])
            offset = (j - n_bars / 2 + 0.5) * width
            ax.text(
                x[i] + offset, 0, f"CL={cl}",
...
def fig_metric_by_users(
    ...
    fig, ax = plt.subplots(figsize=(max(8, len(user_counts) * 1.6), 5.5))
...
def fig_overview_by_users(full_df: pd.DataFrame, out: Path):
    ...
    metrics = [
        ...
        ("on_time_ratio",        "On-Time Ratio",       (0, 1.05), ".3f"),
    ...
    fig, axes = plt.subplots(1, n, figsize=(6.5 * n, 5.5), sharey=False)
    if n == 1:
        axes = [axes]
...
def main():
    ...
    fig_metric_by_users(
        ...
        out=out_dir / "mean_effective_error_by_users.png",
    )
    ...
    if "on_time_ratio" in full_df.columns:
        fig_metric_by_users(
            ...
            out=out_dir / "ontime_ratio_by_users.png",
            ylim=(0, 1.05),
            fmt=".3f",
        )
    ...
    if "mean_delay_ms" in full_df.columns:
        fig_metric_by_users(
            ...
            out=out_dir / "delay_by_users.png",
            fmt=".2f",
        )
    ...
    fig_overview_by_users(
        full_df,
        out=out_dir / "overview_by_users.png",
    )
