...
**Documentation**: [PCA Compression Pipeline](compression/PCA_Pipeline.md)
...
**Documentation**: [Dataset Generation Pipeline](dataset_generation/Dataset_Generation.md)
...
**Documentation**: [Active Machine Learning Architecture](learning/Classifier_Model.md)
...
`run_comparison_parallel.py` simultaneously spawns massive numbers of network workloads. It contrasts assigning the entire network statically to locked compression limits against activating the live `model` prediction pipeline allowing every node to adapt dynamically to congestion.
- **Outputs**: Precise telemetry establishing exactly how much the adaptive AI model improved the QoE reliability ceiling.

**Documentation**: [Simulation Comparison Engine](comparison/Comparison_Pipeline.md)
...
1. **Profile Video Traffic**: Evaluate how differing components shrink payloads *(Compression Pipeline)*.
   `python compression/pca/main.py`
...
4. **Clean Results**: Final prep.
   `python clean_dataset.py datasets/pca/dataset.csv datasets/pca/dataset.csv`
...
6. **Deploy & Validate**: Host the model via FastAPI, and trigger the side-by-side performance parallel comparison. *(Comparison)*.
   `python comparison/run_comparison_parallel.py`