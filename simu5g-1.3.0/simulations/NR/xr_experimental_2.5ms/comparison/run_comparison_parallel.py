...
COMPARISON_TRAFFIC_SUBPATH = "comparison/traffic_files"
BASELINE_TRAFFIC_SUBPATH = "compression/traffic_files"
...
FPS_OPTIONS = [45, 60, 72, 90, 120]            # Per-user frame rate choices - Moderate load
...
def assemble_comparison(
    ...
    if model_data:
        ...
        for i in range(num_users):
            if i in model_data:
                df = model_data[i]
                summary = read_user_summary(model_run_dir, i)
    ...
    for cl, user_dfs in sorted(static_data.items()):
        ...
        for i in range(num_users):
            if i in user_dfs:
                df = user_dfs[i]
                summary = read_user_summary(static_run_dir, i)
                missing_only_lost = count_missing_only_lost_rows(df)
...
def main():
    ...
    parser.add_argument("--traffic-dir", default=None,
                        help="Path to comparison traffic files (default: comparison/traffic_files/*)")
    ...
    if args.traffic_dir:
        ...
        TRAFFIC_DIR = SCRIPT_DIR.parent / f"{COMPARISON_TRAFFIC_SUBPATH}/{MODE}"

    # If the comparison folder does not exist, fall back to the baseline traffic files but warn.
    if not TRAFFIC_DIR.exists():
        fallback_dir = SCRIPT_DIR.parent / f"{BASELINE_TRAFFIC_SUBPATH}/{MODE}"
