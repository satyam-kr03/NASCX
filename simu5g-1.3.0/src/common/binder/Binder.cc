...
namespace simu5g
{
    ...
    void Binder::unregisterNode(MacNodeId id)
    {
        ...
        // Clear XR metrics for the unregistered node
        clearXRMetrics(id);

        // remove 'id' from ulTransmissionMap_ if currently scheduled
        for (auto &carrier : ulTransmissionMap_)
        {
            for (auto &bands : carrier.second)
            {
                for (auto &ues : bands)
                {
    ...
    void Binder::initialize(int stage)
    {
        if (stage == inet::INITSTAGE_LOCAL)
        {
            ...
            // Initialize XR metrics freshness threshold
            xrMetricsFreshnessThreshold_ = par("xrMetricsFreshnessThreshold").doubleValue();
        }
    ...
    {
        if (src < UE_MIN_ID || (src >= MacNodeId(macNodeIdCounter_[1]) && src < NR_UE_MIN_ID) || src >= MacNodeId(macNodeIdCounter_[2]) || dst < UE_MIN_ID || (dst >= MacNodeId(macNodeIdCounter_[1]) && dst < NR_UE_MIN_ID) || dst >= MacNodeId(macNodeIdCounter_[2]))
    ...
    void Binder::addUeCollectorToEnodeB(MacNodeId ue, UeStatsCollector *ueCollector, MacNodeId cell)
    {
        ...
        {
            EV << "LteBinder::addUeCollector - eNodeB [" << cell << "] does not have the eNodeBStatsCollector." << " UeCollector for node [" << ue << "] NOT added to eNodeB [" << cell << "]" << endl;
        }
    }
    ...
    /*
     * XR Traffic Support - Implementation
     */

    void Binder::setXRMetrics(MacNodeId nodeId, int frameNumber, double mse, int sizeBytes)
    {
        Enter_Method_Silent("setXRMetrics");
    ...
    double Binder::getXRMse(MacNodeId nodeId) const
    {
        ...
        {
            return it->second.mse;
        }
        return 0.0;
    }

    double Binder::getXRRmse(MacNodeId nodeId) const
    {
        double mse = getXRMse(nodeId);
        return (mse > 0.0) ? sqrt(mse) : 0.0;
    }

    int Binder::getXRFrameNumber(MacNodeId nodeId) const
    {
        auto it = xrMetrics_.find(nodeId);
        if (it != xrMetrics_.end())
        {
    ...
    int Binder::getXRFrameSize(MacNodeId nodeId) const
    {
        auto it = xrMetrics_.find(nodeId);
        if (it != xrMetrics_.end())
        {
    ...
    bool Binder::isXRMetricFresh(MacNodeId nodeId, double maxAge) const
    {
        auto it = xrMetrics_.find(nodeId);
        if (it == xrMetrics_.end())
        {
        ...
        // Use provided maxAge or default threshold
        double threshold = (maxAge > 0.0) ? maxAge : xrMetricsFreshnessThreshold_;

        simtime_t age = simTime() - it->second.timestamp;
        return age.dbl() < threshold;
    }

    void Binder::clearXRMetrics(MacNodeId nodeId)
    {
        Enter_Method_Silent("clearXRMetrics");

        auto it = xrMetrics_.find(nodeId);
        if (it != xrMetrics_.end())
        {
            EV << "Binder::clearXRMetrics - Clearing metrics for UE " << nodeId << endl;
            xrMetrics_.erase(it);
        }
    }

    const XRMetrics *Binder::getXRMetrics(MacNodeId nodeId) const
    {
        auto it = xrMetrics_.find(nodeId);
        if (it != xrMetrics_.end())
        {
...
namespace simu5g {
void Binder::setXRErrorMetrics(MacNodeId nodeId, const std::vector<double>& errorVector)
{
    if (xrVideoStats_.find(nodeId) != xrVideoStats_.end()) {
        xrVideoStats_[nodeId].currentErrorVector = errorVector;
    }
}
}
