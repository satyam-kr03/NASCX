...
namespace simu5g
{
    ...
    /**
     * Structure to hold XR traffic metrics for scheduling decisions
     */
    struct XRMetrics
    {
        int frameNumber;
    ...
    struct XRVideoStats
    {
        ...
        std::vector<double> currentErrorVector;  // MSE at each CL [5,10,...,80] (16 elements)

        static const int NUM_CL_LEVELS = 16;

        XRVideoStats() : meanTrafficSize(0), stdTrafficSize(0), frameRate(60), prevDelayMs(10.0), bufferBytes(0), mcsIndex(0), currentErrorVector(NUM_CL_LEVELS, 0.0) {}
        XRVideoStats(double m, double s, double fr) : meanTrafficSize(m), stdTrafficSize(s), frameRate(fr), prevDelayMs(10.0), bufferBytes(0), mcsIndex(0), currentErrorVector(NUM_CL_LEVELS, 0.0) {}
    };
    ...
    public:
        ...
        /**
         * Updates XR metrics for a given UE
         * Should be called by XRTrafficSource when sending frames
         *
         * @param nodeId MacNodeId of the UE
         * @param frameNumber Frame sequence number
         * @param mse Mean Squared Error of the frame
         */
        void setXRMetrics(MacNodeId nodeId, int frameNumber, double mse, int sizeBytes);

        /**
         * Retrieves the MSE for a given UE
         *
         * @param nodeId MacNodeId of the UE
         * @return MSE value, or 0.0 if no metrics available
         */
        double getXRMse(MacNodeId nodeId) const;

        /**
         * Retrieves the RMSE for a given UE
         *
         * @param nodeId MacNodeId of the UE
         * @return RMSE value (sqrt of MSE), or 0.0 if no metrics available
         */
        double getXRRmse(MacNodeId nodeId) const;

        /**
         * Retrieves the frame number for a given UE
         *
         * @param nodeId MacNodeId of the UE
         * @return Frame number, or -1 if no metrics available
         */
        int getXRFrameNumber(MacNodeId nodeId) const;

        /**
         * Retrieves the frame size in bytes for a given UE
         *
         * @param nodeId MacNodeId of the UE
         * @return Frame size in bytes, or 0 if no metrics available
         */
        int getXRFrameSize(MacNodeId nodeId) const;

        /**
         * Checks if XR metrics for a UE are fresh (recent)
         *
         * @param nodeId MacNodeId of the UE
         * @param maxAge Maximum age in seconds (default: use threshold from NED)
         * @return true if metrics are fresh, false otherwise
         */
        bool isXRMetricFresh(MacNodeId nodeId, double maxAge = -1.0) const;

        /**
         * Clears XR metrics for a given UE (e.g., when UE disconnects)
         *
         * @param nodeId MacNodeId of the UE
         */
        void clearXRMetrics(MacNodeId nodeId);

        /**
         * Gets the complete XRMetrics structure for a UE
         * Returns nullptr if no metrics available
         *
         * @param nodeId MacNodeId of the UE
        ...
        void setXRErrorMetrics(MacNodeId nodeId, const std::vector<double>& errorVector);
