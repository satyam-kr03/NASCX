...
namespace simu5g
{
    ...
    {
        std::cout << NOW << " LteMaxCI::schedule " << eNbScheduler_->mac_->getMacNodeId() << endl;
        ...
        for (MacCid cid : carrierActiveConnectionSet_)
        {
            ...
            std::cout << NOW << " LteMaxCI::schedule computed for cid " << cid << " score of " << desc.score_ << endl;
        }

        if (direction_ == UL || direction_ == DL)
        { // D2D background traffic not supported (yet?)
        ...
        while (!score.empty())
        {
            ...
            {
                std::cout << NOW << " LteMaxCI::schedule scheduling background UE " << MacCidToNodeId(current.x_) << " with score of " << current.score_ << endl;

                // Grant data to that background connection.
                granted = requestGrantBackground(current.x_, 4294967295U, terminate, active, eligible);

                std::cout << NOW << "LteMaxCI::schedule granted " << granted << " bytes to background UE " << MacCidToNodeId(current.x_) << endl;
            }
            else
            {
                std::cout << NOW << " LteMaxCI::schedule scheduling connection " << current.x_ << " with score of " << current.score_ << endl;

                // Grant data to that connection.
                granted = requestGrant(current.x_, 4294967295U, terminate, active, eligible);

                std::cout << NOW << "LteMaxCI::schedule granted " << granted << " bytes to connection " << current.x_ << endl;
            }
            ...
            {
                score.pop();
                std::cout << NOW << "LteMaxCI::schedule connection " << current.x_ << " was found ineligible" << endl;
            }

            // Set the connection as inactive if indicated by the grant.
            if (!active)
            {
                std::cout << NOW << "LteMaxCI::schedule scheduling connection " << current.x_ << " set to inactive " << endl;
