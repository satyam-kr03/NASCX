...
namespace simu5g
{
    ...
    {
        std::cout << NOW << "LtePf::execSchedule ############### eNodeB " << eNbScheduler_->mac_->getMacNodeId() << " ###############" << endl;
        std::cout << NOW << "LtePf::execSchedule Direction: " << ((direction_ == DL) ? " DL " : " UL ") << endl;
        ...
        for (const auto &cid : carrierActiveConnectionSet_)
        {
            ...
            if (binder_->getOmnetId(nodeId) == 0)
            {
                activeConnectionTempSet_.erase(cid);
                carrierActiveConnectionSet_.erase(cid);
                std::cout << "CID " << cid << " of node " << nodeId << " removed from active connection set - no OmnetId in Binder known.";
                continue;
            }
            ...
            std::cout << NOW << "LtePf::execSchedule CID " << cid << "- Score = " << s << endl;
        }
        ...
        while (!score.empty())
        {
            ...
            std::cout << NOW << "LtePf::execSchedule @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
            std::cout << NOW << "LtePf::execSchedule CID: " << cid;
            std::cout << NOW << "LtePf::execSchedule Score: " << current.score_ << endl;
            ...
            std::cout << NOW << "LtePf::execSchedule Granted: " << granted << " bytes" << endl;

            // Exit immediately if the terminate flag is set.
            if (terminate)
            {
                std::cout << NOW << "LtePf::execSchedule TERMINATE " << endl;
                break;
            }
            ...
            if (!active || !eligible)
            {
                ...
                if (!eligible)
                    std::cout << NOW << "LtePf::execSchedule NOT ELIGIBLE " << endl;
            }

            // Set the connection as inactive if indicated by the grant ().
            if (!active)
            {
                std::cout << NOW << "LtePf::execSchedule NOT ACTIVE" << endl;
    ...
    void LtePf::commitSchedule()
    {
        ...
        {
            std::cout << NOW << " LtePf::storeSchedule @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
            std::cout << NOW << " LtePf::storeSchedule CID: " << cid << endl;
            std::cout << NOW << " LtePf::storeSchedule Direction: " << ((direction_ == DL) ? "DL" : "UL") << endl;
            ...
            std::cout << NOW << " LtePf::storeSchedule Short Term Rate " << shortTermRate << endl;

            // Updating the long term rate
            double &longTermRate = pfRate_[cid];
            longTermRate = (1.0 - pfAlpha_) * longTermRate + pfAlpha_ * shortTermRate;

            std::cout << NOW << "LtePf::storeSchedule Long Term Rate = " << longTermRate;
        }
